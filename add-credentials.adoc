...
各ワークロード機能に必要な権限の詳細については、link:permissions-reference.html["権限リファレンス"]を参照してください。
...
NOTE: GovCloudアカウントをお持ちの場合、ストレージの**表示、計画、および分析**ポリシーにおいて、Cost Explorer の権限 `ce:GetCostAndUsage`または `ce:GetTags`は必要ありません。コードボックスからスクリプトをコピーしたら、これらの権限を削除し、前の行の末尾のカンマを削除してください。
...
NOTE: CloudFormationを使用してAWS GovCloud（US）アカウントにクレデンシャルを追加することはサポートされていません。
