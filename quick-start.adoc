...
[role="quick-margin-para"]
この手順は任意です。Workload Factoryは、AWSアカウントにアクセスするための認証情報を追加しなくても使用できます。Workload FactoryにAWS認証情報を追加すると、Workload FactoryアカウントにFSx for ONTAPファイルシステムの作成と管理、およびデータベースなどの特定のワークロードのデプロイと管理に必要な権限が付与されます。
