...
---
= NetApp Workload Factory でサービスアカウントを管理する
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
. オプションで、クライアント資格情報交換を実行して、Auth0 管理 API のアクセストークンを取得できます。リフレッシュトークンの生成に関する情報については、link:https://docs.netapp.com/us-en/console-automation/platform/grant_types.html#refresh-token["NetApp Console APIドキュメント"^]を参照してください。
+
curl の例は、クライアント ID とシークレットを取得し、API を使用して時間制限のあるアクセストークンを生成する方法を示しています。トークンは、NetApp Workload Factory API への数時間のアクセスを提供します。
...
----
+
出力をhttps://netapp-cloud-account.auth0.com/oauth/tokenに投稿してアクセストークンを取得します。
