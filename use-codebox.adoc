...
Codebox は、各ユーザーの Workload Factory アカウントに提供される AWS 権限に基づいてコードに適切なデータを入力することで、Workload Factory の権限ポリシーに準拠しています。コードはテンプレートのように使用でき、不足している情報 (資格情報など) を入力したり、コードを実行する前に特定のデータをカスタマイズしたりできます。
